...
def format_for_email(config: Any, output: Any) -> Tuple[str, bool]:
    ...
    def fmt_poster_cleanarr(output: dict) -> str:
        """Format poster_cleanarr output for email (HTML)."""
        sections: List[str] = []
        mode = output.get("mode", "unknown").capitalize()
        sections.append(f"<div class='section'><h3>Mode: {mode}</h3>")

        bloat = output.get("bloat", {})
        if bloat.get("count", 0) > 0:
            sections.append(
                f"<p><strong>Bloat Images:</strong> {bloat['count']} files "
                f"({bloat.get('size_human', '0 B')})</p>"
            )

        orphaned = output.get("orphaned", {})
        if orphaned.get("count", 0) > 0:
            sections.append(
                f"<p><strong>Orphaned Posters:</strong> {orphaned['count']}</p>"
            )
        ...
        if maintenance:
            sections.append("<p><strong>Plex Maintenance:</strong></p><ul>")
            for task, status in maintenance.items():
                icon = "&#x2705;" if status == "success" else "&#x274C;"
