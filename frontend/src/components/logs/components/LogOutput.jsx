...
export const LogOutput = React.memo(
    ({ logText, searchTerm, activeLevels, hideHeartbeat, sortNewestFirst = false }) => {
        ...
        if (filteredBlocks.length === 0) {
            return (
                <div className="flex-1 overflow-y-auto font-mono text-sm p-3 bg-[#0e0a22] scrollbar-hidden">
        ...
        return (
            <div
                ref={scrollParentRef}
                className="flex-1 overflow-y-auto font-mono text-sm p-3 bg-[#0e0a22] scrollbar-hidden"
            >
                <div
                    style={{
                        height: `${totalSize}px`,
