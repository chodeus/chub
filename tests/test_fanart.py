...
and the in-run memo (there is no persistent cache).
"""
...
def _client(client_key="personal"):
    return FanartClient(FanartConfig(client_key=client_key), _logger())
...
def test_disabled_without_client_key():
    c = _client(client_key="")
    assert c.enabled is False
    assert c.get_images({"asset_type": "movie", "tmdb_id": 1}) is None


def test_enabled_with_client_key():
    assert _client().enabled is True


def test_logo_prefers_language_then_likes():
    c = _client()
...
def test_logo_hd_preferred_over_sd():
    c = _client()
    c._fetch = lambda kind, lid: MOVIE_RAW
    images = c.get_images({"asset_type": "movie", "tmdb_id": 123}, language=["en"])
...
def test_background_prefers_textless():
    c = _client()
    c._fetch = lambda kind, lid: MOVIE_RAW
    images = c.get_images({"asset_type": "movie", "tmdb_id": 123}, language=["en"])
...
def test_logo_falls_back_to_sd_when_no_hd():
    c = _client()
...
def test_tv_uses_tvdb_and_scopes_season_background():
    raw = {
...
def test_show_level_background_prefers_all():
    raw = {
        "showbackground": [
...
def test_movie_falls_back_to_imdb_id():
    captured = {}
    c = _client()
...
def test_in_run_memo_avoids_refetch():
    """The per-run memo collapses duplicate work (same item resolved for logo
    and background in one run) — but there is no persistent cache, so a fresh
    client re-fetches."""
    calls = {"n": 0}
    c = _client()
    ...
    c.get_images(media)
    c.get_images(media)  # same item, same run -> memo hit, no refetch
    assert calls["n"] == 1

    # A fresh client has no memo and no persistent cache -> fetches again.
    c2 = _client()
...
def test_collection_returns_none():
    c = _client()
